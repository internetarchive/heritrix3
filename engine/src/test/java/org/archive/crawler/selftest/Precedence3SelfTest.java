 ...
 * 
 * <p>
 * The test data consists of 15 documents, titled A through O. Each document
 ...
 * 
 * <p>
 * If H is the seed, then Heritrix would ordinarily crawl these in the order
 * <code>(H, L, D, J, N, F, B, K, I, M, O, G, E, C, A)</code> -- loosely the
 * order the links were discovered.
 * 
 * <p>
 * However, this test uses the {@link KeyWordProcessor} to ensure that, if a
 * document contains a certain keyword, then that document's out links are
 * crawled before the out links of documents that do not contain the keyword.
 * 
 * <p>
 * The documents A, B, D, E, H, I, J and M all contain the keyword (these are
 * the "left-branch"/first-link documents in the tree above, plus the
 * root/seed). The other documents do not.
 * 
 * <p>
 * Therefore this test makes sure that the children of documents containing the
 * keyword are crawled before children of documents not containing the keyword:
 * 
 * <ol>
 * <li>The children of D (B and F) should be crawled before the children of L (J
 * and N).</li>
 * <li>The children of B (A and C) should be crawled before the children of F (E
 * and G).</li>
 * <li>The children of J (I and K) should be crawled before the children of N (M
 * and O).
 * </ol>
 * 
 * <p>
 * This test provides a simple proof-of-concept that shows how the content of
 * one URI can alter the precedence of the out links of that URI. See
